...
def train_epoch(model, loader, optimizer):
    # déclencher le train mode du model
    model.train()
    train_loss = []
    bar = tqdm(loader)
    # le loader nous permet de récupérer la data et la target
    for (data, target) in bar:

        # vider les gradients
        optimizer.zero_grad()
        
        # dans le cas on utilise image + meta_feature
        if args.use_meta:
            data, meta = data
            data, meta, target = data.to(device), meta.to(device), target.to(device)
            logits = model(data, meta)
        else:
            #dans le cas on utilise uniqument les  images
...
def val_epoch(model, loader, mel_idx, is_ext=None, n_test=1, get_output=False):
    # lancer le mode evaluation
    model.eval()
    # creer une matrice  pour stocker le resultat de l'évaluation
    val_loss = []
    LOGITS = []
    PROBS = []
    TARGETS = []
